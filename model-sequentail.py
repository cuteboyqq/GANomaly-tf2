...
class Encoder(tf.keras.layers.Layer):
    ...
    def __init__(self,
        ...
        self.conv_in = layers.Conv2D(filters=ndf,
                                kernel_size=4,
                                strides=2,
                                padding='same',
                                use_bias=False)
        
        self.act_in = layers.PReLU(shared_axes=[1,2])
        
        #self.body_blocks.append(body)
        #=================================
        #self.in_blocks.append(conv)
        #self.in_blocks.append(act)
        #=================================
        ...
        for t in range(n_extra_layers):
            ...
            conv = layers.Conv2D(filters=cndf,
                                    kernel_size=3,
                                    strides=2,
                                    padding='same',
                                    use_bias=False)
            
            bn = layers.BatchNormalization(epsilon=1e-05, momentum=0.9)
            
            act = layers.PReLU(shared_axes=[1,2])
            
            self.extra_blocks.append(conv)
            self.extra_blocks.append(bn)
            self.extra_blocks.append(act)
        ...
        body = Conv_BN_Act(filters=128,
                           ...
                           strides=2)
        '''
        self.conv_b1 = layers.Conv2D(filters=128,
                                kernel_size=4,
                                strides=2,
                                padding='same',
                                use_bias=False)
        
        self.bn_b1 = layers.BatchNormalization(epsilon=1e-05, momentum=0.9)
        
        self.act_b1 = layers.PReLU(shared_axes=[1,2])
        
        #self.body_blocks.append(body)
        #=============================
        '''
        self.body_blocks.append(conv)
        self.body_blocks.append(bn)
        self.body_blocks.append(act)
        '''
        #============================
        #8
        '''
        body = Conv_BN_Act(filters=256,
                           ...
                           strides=2)
        '''
        self.conv_b2 = layers.Conv2D(filters=256,
                                kernel_size=4,
                                strides=2,
                                padding='same',
                                use_bias=False)
        
        self.bn_b2 = layers.BatchNormalization(epsilon=1e-05, momentum=0.9)
        
        self.act_b2 = layers.PReLU(shared_axes=[1,2])
    ...
    def call(self, x):
        ...
        return out
#===============================================================================================================
class Encoder_forD(tf.keras.layers.Layer):
    """ DCGAN ENCODER NETWORK
    """
    def __init__(self,
                 isize,
        ...
        Params:
            ...
            ndf(int): num of discriminator(Encoder) filters
        """
        super(Encoder_forD, self).__init__()
        assert isize % 16 == 0, "isize has to be a multiple of 16"
...
class Decoder(tf.keras.layers.Layer):
    def __init__(self, isize, nz, nc, ngf, n_extra_layers=0):
        ...
        self.in_block = Conv_BN_Act(filters=cngf,
                                    ...
                                    conv_tran=True)
        '''
        self.conv_tran_inblock1 = layers.Conv2DTranspose(filters=cngf,
                                           kernel_size=4,
                                           strides=1,
                                           padding='valid',
                                           use_bias=False)
        ...
        '''
        conv = layers.Conv2D(filters=cngf,
                                kernel_size=4,
                                strides=1,
                                padding='same',
                                use_bias=False)
        
        conv_tran = layers.UpSampling2D()
        conv_tran = layers.UpSampling2D()
        '''
        self.bn_inblock1 = layers.BatchNormalization(epsilon=1e-05, momentum=0.9)
        
        self.act_inblock1 = layers.ReLU()
        
        #self.body_blocks.append(body)
        #self.in_blocks.append(conv)
        #self.in_blocks.append(conv_tran)
        #self.in_blocks.append(conv_tran)
        #self.in_blocks.append(bn)
        #self.in_blocks.append(act)
        #======================================================================
        csize, _ = 4, cngf
        #================
        ...
        self.conv_tran_body1 = layers.Conv2DTranspose(filters=128,
                                           kernel_size=4,
                                           strides=2,
                                           padding='same',
                                           use_bias=False)
        ...
        #conv_tran = layers.UpSampling2D()
        '''
        conv = layers.Conv2D(filters=128,
                                kernel_size=4,
                                strides=1,
                                padding='same',
                                use_bias=False)
        '''
        self.bn_body1 = layers.BatchNormalization(epsilon=1e-05, momentum=0.9)
        
        self.act_body1 = layers.ReLU()
        
        #self.body_blocks.append(body)
        
        #self.body_blocks.append(conv_tran)
        #self.body_blocks.append(conv)
        ...
        self.conv_tran_body2 = layers.Conv2DTranspose(filters=64,
                                           kernel_size=4,
                                           strides=2,
                                           padding='same',
                                           use_bias=False)
        ...
        conv = layers.Conv2D(filters=64,
                                ...
                                use_bias=False)
        '''
        self.bn_body2 = layers.BatchNormalization(epsilon=1e-05, momentum=0.9)
        
        self.act_body2 = layers.ReLU()
        ...
        #for t in range(n_extra_layers):
            ...
            #self.conv_tran_extra1 = layers.Conv2DTranspose(filters=cngf,
            #                                   kernel_size=3,
            #                                   strides=1,
            #                                   padding='same',
            #                                   use_bias=False)
            ...
            #conv_tran = layers.UpSampling2D()
            #self.bn_extra1 = layers.BatchNormalization(epsilon=1e-05, momentum=0.9)
            
            #self.act_extra1 = layers.ReLU()
            
            #self.body_blocks.append(body)
            #self.extra_blocks.append(conv_tran)
            #self.extra_blocks.append(bn)
            #self.extra_blocks.append(act)    
        ...
        self.conv_tran_outblock1 = layers.Conv2DTranspose(filters=nc,
                                           kernel_size=4,
                                           strides=2,
                                           padding='same',
                                           use_bias=False)
        ...
        #conv_tran = layers.UpSampling2D()
        '''
        conv = layers.Conv2D(filters=3,
                                kernel_size=4,
                                strides=1,
                                padding='same',
                                use_bias=False)
        '''
        self.act_outblock1 = layers.Activation(tf.tanh)
        
        
        #self.out_blocks.append(conv_tran)
        #self.out_blocks.append(conv)
        #self.out_blocks.append(act)    
        #=====================================================================
    def call(self, x):
        #x = self.in_block(x)
        #====================================
        #for block in self.in_blocks:
            #x = block(x)
        x=self.conv_tran_inblock1(x)
        x=self.act_inblock1(x)
        #x=self.bn_inblock1(x)

        #====================================
        #for block in self.body_blocks:
        x=self.conv_tran_body1(x)
        x=self.act_body1(x)
        #x=self.bn_body1(x)
        
        x=self.conv_tran_body2(x)
        x=self.act_body2(x)
        #x=self.bn_body2(x)
        #====================================
        #for block in self.extra_blocks:
        ...
        #for block in self.out_blocks:
            #x = block(x)
        x=self.conv_tran_outblock1(x)
        x=self.act_outblock1(x)
...
class NetD(tf.keras.Model):
    ...
    def __init__(self, opt):
        ...
        if opt.encdims:
            ...
            self.encoder = Encoder_forD(opt.isize, 1, opt.nc, opt.ngf, opt.extralayers, output_features=True)
